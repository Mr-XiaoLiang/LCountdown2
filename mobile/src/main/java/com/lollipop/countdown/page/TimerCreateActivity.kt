...
import com.lollipop.countdown.databinding.ActivityTimerCreateBinding
import com.lollipop.countdown.databinding.ItemTimeSpareBinding
import com.lollipop.countdown.helper.TimerManager
import java.util.Calendar

class TimerCreateActivity : BasicActivity() {

    private val binding by lazy {
        ActivityTimerCreateBinding.inflate(layoutInflater)
    }

    private val timeList = ArrayList<ItemInfo>()
    ...
    override fun onResume() {
        ...
        TimeCache.fetchCache().forEach {
            timeList.add(ItemInfo.Time(it))
        }
        timeList.add(ItemInfo.Space)
        itemAdapter.notifyDataSetChanged()
        Toast.makeText(this, "timeList.size = ${timeList.size}", Toast.LENGTH_SHORT).show()
    }
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemHolder {
            when (viewType) {
                TYPE_TIME -> {
                    ...
                    return ItemHolder.Empty.create(parent, 84)
                }
            }
        }
